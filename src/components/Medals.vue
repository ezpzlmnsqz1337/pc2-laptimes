...
export default {
  ...
  methods: {
    ...
    getRank (driverId, position) {
      ...
      const maxBonus = this.$sb.calculateBonus(this.medals[0], this.totalRaces[0].races)
      const maxPoints = driverTotalRaces.races / this.$sb.calculatePoints(this.medals[0]) * 10 + maxBonus
